...
export default {
  ...
  mounted(){
    ...
    // Hämta parent element strolek
    var parent = d3.select('#cont').node().getBoundingClientRect();
    console.log(parent);

    //Scatterplot
    var margin = {left: 20, top: 4, right: 20, bottom: 4},
    	width = parent.width - margin.left - margin.right,
    	height = parent.width - margin.bottom;
    ...
    var xScale = d3.scaleLog()
    	...
    	.domain(d3.extent(streams, function(d) { return d.GDP_perCapita; }))
    	.nice();
    //Set new x-axis
    ...
    var yScale = d3.scaleLinear()
    	.range([height,0])
    	.domain(d3.extent(streams, function(d) { return d.lifeExpectancy; }))
    	.nice();	
    ...
    var rScale = d3.scaleSqrt()
    			.range([2,16])
    			.domain(d3.extent(streams, function(d) { return d.GDP; }));
    ...
    circleGroup.selectAll("streams")
    	.data(streams) //Sort so the biggest circles are below .sort(function(a,b) { return b.GDP > a.GDP; })
    	.enter().append("circle")
    		.attr("class", function(d,i) { return "streams " + d.CountryCode; })
